...
Deltiecord 0.9.25 build 77 is the latest stable release for Android, x86-64 Windows, and
Linux.
Download official builds from the [Deltiecord releases page](https://deltie.net/cord)
or the [GitHub release](https://github.com/vosjecleo/deltiecord/releases/tag/v0.9.25-b77).
...
`deltiecord-0.9.25+77-android-arm64-v8a.apk`; older 32-bit phones use
...
Download `deltiecord-0.9.25+77-windows-x64-setup.exe`, run it, and follow the
...
Download `deltiecord-0.9.25+77-windows-x64-portable.zip`, extract the entire
...
Download `deltiecord-0.9.25+77-linux-debian-amd64.deb`, open a terminal in its directory, and
install it with APT:
...
Download `deltiecord-0.9.25+77-linux-arch-x86_64.pkg.tar.zst` and install it with pacman:

```sh
sudo pacman -U ./deltiecord-0.9.25+77-linux-arch-x86_64.pkg.tar.zst
```
...
`deltiecord-0.9.25+77-linux-appimage-x86_64.AppImage`, make it executable, and launch it:
