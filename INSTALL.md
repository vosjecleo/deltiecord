...
Deltiecord 0.9.27 build 92 is the current release candidate for Android,
...
`deltiecord-0.9.27+92-android-arm64-v8a.apk`; older 32-bit phones use
...
Download `deltiecord-0.9.27+92-windows-x64-setup.exe`, run it, and follow the
...
Download `deltiecord-0.9.27+92-windows-x64-portable.zip`, extract the entire
...
Download `deltiecord-0.9.27+92-linux-debian-amd64.deb`, open a terminal in its directory, and
install it with APT:

```sh
sudo apt install ./deltiecord-0.9.27+92-linux-debian-amd64.deb
```
...
Download `deltiecord-0.9.27+92-linux-arch-x86_64.pkg.tar.zst` and install it with pacman:

```sh
sudo pacman -U ./deltiecord-0.9.27+92-linux-arch-x86_64.pkg.tar.zst
```
...
`deltiecord-0.9.27+92-linux-appimage-x86_64.AppImage`, make it executable, and launch it:

```sh
chmod +x deltiecord-0.9.27+92-linux-appimage-x86_64.AppImage
./deltiecord-0.9.27+92-linux-appimage-x86_64.AppImage
```
